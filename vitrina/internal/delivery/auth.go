...
func (h *AuthHandler) SignUp(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(err)
	if err != nil {
		if errors.Is(err, repo.ErrWrongLoginOrPassword) {
			response.WithError(w, 500, handler, repo.ErrWrongLoginOrPassword)
			return
		}
		if errors.Is(err, repo.ErrEqualUser) {
			response.WithError(w, 400, handler, repo.ErrEqualUser)
			return
		}
